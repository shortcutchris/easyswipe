...
verify_project() {
  ...
  release_project
  remote "set -eu
    /usr/bin/python3 - '${SUMMARY_FILE}' '${REMOTE_ROOT}/.remote-source-revision' '${STAGED_APP}' '${ARTIFACTS_ROOT}/verification.json' <<'PY'
import datetime
import json
import plistlib
import subprocess
...
if result != 'Passed' or failed != 0 or total <= 0:
    raise SystemExit(
        f'Refusing verification manifest: result={result!r}, '
        f'failedTests={failed}, totalTestCount={total}'
    )
