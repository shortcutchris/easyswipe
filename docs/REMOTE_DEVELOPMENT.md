...
`fetch` also retrieves the command-line tools from the exact pinned Sparkle package resolved by the verified build. `local-sign` runs only on the controlling Mac after `fetch`. It re-signs Sparkle's nested XPC services, helper executable, updater app, framework, and finally EasySwipe with the explicitly supplied Developer ID identity. It verifies the nested signatures, rejects development-only entitlements, runs the startup probe, and records the stable Team ID in an ignored local manifest. Keeping the same Developer ID and bundle identifier lets macOS recognize later builds as the same privacy-sensitive app. Signing identities, private keys, and notarization credentials remain in the local Keychain and are never synchronized to the Studio.
